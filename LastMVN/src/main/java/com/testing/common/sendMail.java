...
/**
 * 可传入的参数：服务器地址，服务器端口，发件人账号、密码，收件人（可多个），抄送人（可多个），报告路径
 */
...
public class sendMail {
	...
	public String myEmailAccount = "652200567@qq.com";
	public String myEmailPassword = "kbabgiitttqsbbde";

	// 发件人邮箱的 SMTP 服务器地址, 必须准确, 不同邮件服务器地址不同, 一般(只是一般, 绝非绝对)格式为: smtp.xxx.com
	// 网易163邮箱的 SMTP 服务器地址为: smtp.163.com QQ邮箱为smtp.qq.com 在各大邮箱的设置中能够找到。
	public String myEmailSMTPHost = "smtp.qq.com";
	// 收件人邮箱（替换为自己知道的有效邮箱）
	public String receiveMailAccount = "3028978180@qq.com,652200567@qq.com";
	// 昵称
	public String user="Ronnie测试";
	// 抄送名单
	public String copyMailAccount = "245766353@qq.com";
	// 标题
	public String title = "测试邮件-来自远古的自动化测试框架！";
	// 属性
	public Properties props;

	/**
	 * 完成邮件初始化设置，为成员变量赋值，如果没有传递外部参数，则使用成员变量中的默认参数
	 * @param map
	 * @return
	 */
	public boolean initMail(Map<String, String> map) {
		...
		// 初始化邮件配置
		try {
			myEmailAccount = map.get("emailAccount");
			myEmailPassword = map.get("emailPassword");
			// 如果未输入host参数，尝试通过邮件地址进行拼接
			if (map.get("host") == null || map.get("host").equals("null") || map.get("host").trim().equals("")) {
	...
	public static void main(String[] args) {
		...
		map.put("emailPassword", "tlshvzjkyebwbdig");
		map.put("sendnick", "自动化小白");
		map.put("host", "smtp.qq.com");
		map.put("recievers", "245766353@qq.com,3028978180@qq.com");
		map.put("copylist", "120517972@qq.com");

		mail.initMail(map);
		try {
			mail.send("这是一封邮件", "C:\\Users\\Administrator\\Desktop\\1233.png");  //不管文本还是图片，都是以附件形式发送
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
