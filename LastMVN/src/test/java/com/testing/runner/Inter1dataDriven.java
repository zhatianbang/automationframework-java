...
//通过数据指定的关键字运行，并且断言
  ...
  public void f(String rowNo, String kong,String casename,String keywords,String param1,String param2,String param3,String k1,String k2,String k3,String k4 ) {
	  ...
	  //行数和用例名称输出
	  AutoLogger.log.info("测试："+rowNo + "  " + casename + "  " +runRes);
  ...
  public Object[][] keywords() {
	  return caseExcel.readAsMatrix(0);
  }
...
//	  htmlReport.sendreport(rootpath+"\\cases\\\\result\\result-"+createdate+"WebCases2.xlsx",createdate);
  ...
  private String runUIWithInvoke(String key,String param1,String param2,String param3) {
	  	String result ="fail";
		try {
			Method appMethod = http.getClass().getDeclaredMethod(key);
			// invoke语法，需要输入类名以及相应的方法用到的参数
			result=appMethod.invoke(http).toString();
			return result;
		} catch (Exception e) {
		}
		try {
			Method uis = http.getClass().getDeclaredMethod(key, String.class);
			// invoke语法，需要输入类名以及相应的方法用到的参数
			result=uis.invoke(http, param1).toString();
			return result;
		} catch (Exception e) {
		}
		try {
			Method uis = http.getClass().getDeclaredMethod(key, String.class, String.class);
			// invoke语法，需要输入类名以及相应的方法用到的参数
			result=uis.invoke(http, param1, param2).toString();
			return result;
		} catch (Exception e) {
		}
		try {
			Method uis = http.getClass().getDeclaredMethod(key, String.class, String.class, String.class);
			// invoke语法，需要输入类名以及相应的方法用到的参数
			result=uis.invoke(http, param1, param2,param3).toString();
			return result;
		} catch (Exception e) {
		}
		return result;
